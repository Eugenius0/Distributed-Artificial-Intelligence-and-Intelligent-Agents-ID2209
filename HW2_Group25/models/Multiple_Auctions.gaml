/***
* Name: Multiple Auctions
* Author: Dominika Drela, Eugen Lucchiari Hartz, Group 25
* Description: Challenge 2 : Multiple auctions: Dutch, English and Sealed
***/
...
global {
	...
	int auctioneerMinimumValueMin <- 90;
	int auctioneerMinimumValueMax <- 300;
	...
	init {
		...
		/*
		 * Number of auctioneers is defined above 
		 */
...
species Guest skills:[moving, fipa] {		
	...
	/*
	 * In Dutch auction, the auctioneer proposes and the participant can accept or reject it, based on the price it would pay for it.
	 */
...
/*
 * The AuctionMaster creates auctioneers
 */
species AuctionMaster skills:[fipa] {
	bool auctioneersCreated <- false;
	list<Auctioneer> auctioneers <- [];
///	/*
//	 * This creates the auctioneers within the set time limits from the beginning.
...
species Auctioneer skills:[fipa, moving] {
	...
	/*
	 * sets auctionStarted to true when interestedGuests are within a distance of 13 to the auctioneer.
	 */
	...
	/*
	 * Dutch auction: auctioneer sends a propose message and guests can reply with accept or reject messages. The auction ends with the first accept.
	 */
	reflex receiveAcceptMessages when: auctionRunning and !empty(accept_proposals) {
		if(auctionType = "Dutch") {
			...
			interestedGuests <- [];
			DeadAuctioneers<- DeadAuctioneers + 1;
			do die;
		}
	}

	/*
	 * In sealed and english auction, the participants send proposes to the auctioneer. The auctioneer gets them here.
	...
	reflex receiveRejectMessages when: auctionRunning and !empty(reject_proposals) {
		if(auctionType = "Dutch") {
			...
			if(auctioneerDutchPrice < auctioneerMinimumValue) {
				...
				write name + ' price went below minimum value (' + auctioneerMinimumValue + '). It is not feasible to continue!';
				do start_conversation (to: interestedGuests, protocol: 'fipa-propose', performative: 'cfp', contents: ['Stop']);
				do die;
				DeadAuctioneers<- DeadAuctioneers + 1;				
				interestedGuests <- [];
			...
			if(length(interestedGuests) < 2) {
				...
				if(currentBid < auctioneerMinimumValue) {
	...
	reflex sendAuctionInfo when: auctionRunning and time >= 50 and !empty(interestedGuests){
		if(auctionType = "Dutch") {
			write name + ' sends the offer of ' + auctioneerDutchPrice +' kronas to participants';
			...
			write 'Auctioneer ' + name + ': current bid is: ' + currentBid + '. Offer more or miss your chance!';
	...
	reflex CheackDead when: DeadAuctioneers>= 3 {
		ask AuctionMaster{
			self.auctioneersCreated<- false;
			write " #################### all dead";
		}
	} 
