...
* Description: Assignment 2, simulation of Dutch auction
***/
...
species AuctionCreator skills:[fipa] {
	...
	reflex create_Auctioneers when: !auctioneersCreated and time rnd(auctionCreationMin, auctionCreationMax) {
...
species Auctioneer skills:[fipa, moving] {
	...
	reflex receive_Reject_Messages when: auctionRunning and !empty(reject_proposals) {
		if(auctionType = "Dutch") {
			...
			auctioneerDutchPrice <- auctioneerDutchPrice - rnd(dutchAuctionDecrementMin, dutchAuctionDecrementMax);
	...
	reflex sendAuctionInfo when: auctionRunning and time >= 50 and !empty(interestedGuests){
		if(auctionType = "Dutch") {
			...
			do start_conversation (to: interestedGuests, protocol: 'fipa-propose', performative: 'propose', contents: ['Buy my merch, peasant', auctionType, auctioneerDutchPrice]);
		}
	}	
}// Auctioneer
